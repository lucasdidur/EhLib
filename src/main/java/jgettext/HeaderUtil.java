...
public class HeaderUtil
{
    ...
    public static Message generateDefaultHeader(Date potCreationDate)
    {
        ...
        param.comments
                .add("This file is distributed under the same license as the PACKAGE package.");
    ...
    private static void setValues(HeaderFields header, HeaderValues param)
    {
        ...
        header.setValue(HeaderFields.KEY_ProjectIdVersion,
                param.projectIdVersion);
        header.setValue(HeaderFields.KEY_ReportMsgidBugsTo,
                param.reportMsgidBugsTo);
        header.setValue(HeaderFields.KEY_PotCreationDate,
                dateFormat.format(param.potCreationDate));
        ...
        header.setValue(HeaderFields.KEY_ContentType, "text/plain; charset="
                + param.charset);
        header.setValue(HeaderFields.KEY_ContentTransferEncoding, "8bit");
    }

    /**
     * This "Parameter object" might eventually morph into our proper Header
     * class.
     * <p>
     * To do that it, we will need HeaderFields.getValues(), and support for
     * arbitrary field names (eg Set<String> otherFields).
     *
     * @author sflaniga
     */
    private static class HeaderValues
    {
        ...
        /**
         * Perhaps poRevisionDate should be a Date too, but null values will be
         * converted to the string "YEAR-MO-DA HO:MI+ZONE" ?
         */
