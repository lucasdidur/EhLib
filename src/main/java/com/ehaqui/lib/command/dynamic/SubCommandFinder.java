package com.ehaqui.lib.command.dynamic;
...
import com.ehaqui.lib.command.CallInfo;
import com.ehaqui.lib.command.MondoCommand;
import com.ehaqui.lib.command.MondoFailure;
import com.ehaqui.lib.command.SubCommand;
import com.ehaqui.lib.command.SubHandler;
...
public class SubCommandFinder
{
    ...
    public void registerMethods(Object handler)
    {
        for (Method method : sortedMethods(handler))
        {
            ...
            {
                logOutput.println(String.format("MondoCommand: @Sub marked on  '%s' from class %s, must receive only one argument of type CallInfo.", method.getName(), handler.getClass().getName()));
                continue;
            }
    ...
    private void registerMethod(Object handler, Method method, Sub subInfo)
    {
        ...
        SubCommand sub = base.addSub(name, permission).setMinArgs(subInfo.minArgs()).setDescription(subInfo.description()).setUsage(subInfo.usage());
