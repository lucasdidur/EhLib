...
public class SingleItemSerialization
{
    ...
    /**
     * Serialize an ItemStack that is inside an inventory. An extra "index" key will be added to store the position of the ItemStack in the inventory
     *
        ...
        try
        {
            ...
            if (hasMeta)
            {
                ...
                if (meta.hasLore())
                {
                    ...
                    for(String tmp : meta.getLore())
                    {
                        l.add(tmp.replace("\"", "\\\""));
                    }

                    lore = l.toArray(new String[]{});
                }
                if (meta.hasEnchants())
                {
    ...
    /**
     * Deserialize an ItemStack. An index if given strictly for debug purposes. When an error message is given the index will be used for more useful reference
     *
     * @param item  The ItemStack to deserialize
    ...
    public static ItemStack getItem(JSONObject item, int index)
    {
        try
        {
            ...
            if (item.has("name"))
            {
                name = item.getString("name").replace("\\\"","\"");
            }
            ...
            if (item.has("lore"))
            {
                ...
                {
                    lore.add(l.getString(j).replace("\\\"","\""));
                }
            }
